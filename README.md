...
`iroquiz/` は、紅・藍・萌黄色といった日本の伝統色の「名前」「特徴」
「トリビア」をクイズ形式で覚えられる、依存ライブラリ不要（標準ライブラリの
`curses` のみ）の TUI アプリです。上記2つのツールとは無関係の独立した
ツールです。
...
- **色名当てクイズ**: 色見本（対応端末では実際の色に近い背景色、非対応端末
  ではカラーコードのテキスト）を見て、4択から正しい色名を選びます。
- **特徴当てクイズ・トリビア当てクイズ**: 色名を見て、その色の特徴／
  トリビアとして正しい説明文を4択から選びます。
- **カテゴリ指定出題**: 赤系・橙系・黄系・緑系・青系・紫系・白黒鼠系の
  7カテゴリから絞り込んで出題できます。
- **苦手な色を優先して出題**: これまでの正答率が低い色ほど出題されやすく
  なります（色名・特徴・トリビアの出題形式もランダムに混ざります）。
- **一覧表示**: 全36色をカテゴリ順に一覧表示します。
- **成績**: 累計の正答率と、正答率が低い色のランキングを確認できます。

成績は既定で `~/.iroquiz_stats.json` に保存されます（`--state-file`
オプションで変更可能）。

収録している36色の名前・読み・カラーコード（目安の近似値）・特徴・
トリビアは、日本の伝統色に関して広く知られている内容（染料・顔料の由来、
歴史的な用途、文学作品での言及など）をもとにまとめたものです。
